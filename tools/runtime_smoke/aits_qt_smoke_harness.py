...
PUBLIC_MARKET_READ_MODES = {
        ...
        "real-user-app-public-feed-profile-summary",
...
def _run_order_intent_one_shot_unlock_readiness_fixture_proof(report: dict[str, Any]) -> None:
    scenarios: list[dict[str, Any]] = [
        ...
        {
            "name": "unlock_consumed_blocks",
            "candidate": _base_source_policy_candidate("user_added", "KRW-PYTH"),
            "session_approved": ["KRW-PYTH"],
            "mock_unlock_approved": ["KRW-PYTH"],
            "mock_unlock_consumed": True,
            "expected_ready": False,
            "expected_blockers": ["one_shot_unlock_consumed"],
        },
        {
            "name": "unlock_reuse_blocks",
            "candidate": _base_source_policy_candidate("user_added", "KRW-PYTH"),
            "session_approved": ["KRW-PYTH"],
            "mock_unlock_approved": ["KRW-PYTH"],
            "mock_unlock_reusable": True,
            "expected_ready": False,
            "expected_blockers": ["one_shot_unlock_reuse_not_allowed"],
        },
        {
            "name": "router_payload_not_ready_blocks",
            "candidate": _base_source_policy_candidate("user_added", "KRW-PYTH"),
            "session_approved": [],
            "mock_unlock_approved": ["KRW-PYTH"],
            "expected_ready": False,
    ...
    report.update(
        {
            "one_shot_unlock_readiness_stub_supported": True,
            "schema": "aits_order_intent_one_shot_unlock_readiness_v1",
            "evaluator_owner": "tools/runtime_smoke/aits_qt_smoke_harness.py::_evaluate_order_intent_one_shot_unlock_readiness_v1",
            "fixture_results": results,
            "fixture_pass_count": sum(1 for item in results if item.get("pass")),
            "fixture_fail_count": sum(1 for item in results if not item.get("pass")),
            "actual_order_intent_emitted": False,
...
def _run_order_intent_one_shot_unlock_readiness_live_proof(
    ...
    *,
    ...
    report.update(
        {
            "mode": "order-intent-one-shot-unlock-readiness-live-proof",
            "one_shot_unlock_readiness_stub_supported": True,
            "schema": "aits_order_intent_one_shot_unlock_readiness_v1",
            "evaluator_owner": "tools/runtime_smoke/aits_qt_smoke_harness.py::_evaluate_order_intent_one_shot_unlock_readiness_v1",
...
def _evaluate_live_preflight_readonly_adapter_contract_v1(
    ...
    *,
    ...
    required_truthy = {
        ...
        "live_order_readiness": "live_order_readiness_not_ready",
    ...
    for key, blocker in required_truthy.items():
        if not bool(adapter_input.get(key)) and blocker not in blockers:
            blockers.append(blocker)

    symbol = _normalize_symbol_text(str(adapter_input.get("symbol") or ""))
    if not symbol:
        blockers.append("symbol_missing")
    side = str(adapter_input.get("side") or "buy")
    ...
    total_window_cap = int(adapter_input.get("total_window_cap_krw") or 20_000)
    total_after = int(adapter_input.get("total_window_after_candidate_krw") or 0)
    ...
    if intended_amount > per_order_cap:
        blockers.append("amount_above_per_order_hard_cap")
    if total_after > total_window_cap:
        blockers.append("total_window_cap_exceeded")
    if submitted_count != 0:
        blockers.append("submitted_nonzero")
    if bool(adapter_input.get("actual_order")):
        blockers.append("actual_order_true")

    forced_false_flags = {
        "would_call_live_preflight": bool(context.get("would_call_live_preflight", False)),
        "live_preflight_called": bool(context.get("live_preflight_called", False)),
        "order_service_reachable": bool(context.get("order_service_reachable", False)),
        "order_adapter_reachable": bool(context.get("order_adapter_reachable", False)),
        "unlock_consumed": bool(context.get("unlock_consumed", False)),
        "actual_order_intent_emitted": bool(context.get("actual_order_intent_emitted", False)),
        "decision_router_called": bool(context.get("decision_router_called", False)),
        "risk_guard_called": bool(context.get("risk_guard_called", False)),
        "order_service_called": bool(context.get("order_service_called", False)),
        "order_adapter_called": bool(context.get("order_adapter_called", False)),
        "unlock_service_called": bool(context.get("unlock_service_called", False)),
        "actual_order": bool(context.get("actual_order", False)),
        "managed_pool_mutation": bool(context.get("managed_pool_mutation", False)),
        "order_risk_detected": bool(context.get("order_risk_detected", False)),
    }
    flag_blockers = {
        "would_call_live_preflight": "would_call_live_preflight_true",
        "live_preflight_called": "live_preflight_called_true",
        "order_service_reachable": "order_service_reachable_true",
        "order_adapter_reachable": "order_adapter_reachable_true",
        "unlock_consumed": "unlock_consumed_true",
        "actual_order_intent_emitted": "actual_order_intent_emitted_true",
        "decision_router_called": "decision_router_called_true",
        "risk_guard_called": "risk_guard_called_true",
        "order_service_called": "order_service_called_true",
        "order_adapter_called": "order_adapter_called_true",
        "unlock_service_called": "unlock_service_called_true",
        "actual_order": "actual_order_true",
        "managed_pool_mutation": "managed_pool_mutation_true",
        "order_risk_detected": "order_risk_detected_true",
    }
    for key, value in forced_false_flags.items():
        if value and flag_blockers[key] not in blockers:
            blockers.append(flag_blockers[key])
    ...
    if str(adapter_input.get("schema") or "") != "aits_live_preflight_readonly_adapter_input_v1":
        blockers.append("adapter_input_schema_invalid")
    if str(adapter_input.get("candidate_schema") or "") == "":
        warnings.append("candidate_schema_missing")

    adapter_ready = bool(not blockers)
    return {
        "schema": "aits_live_preflight_readonly_adapter_contract_v1",
        ...
        "blockers": blockers,
        "warnings": warnings,
        "required_next_goal": "AITS-RISKGUARD-READONLY-ADAPTER-DESIGN-REVIEW-01",
    }


def _validate_live_preflight_readonly_adapter_contract_v1(result: dict[str, Any] | None) -> list[str]:
    if not result:
        return ["readonly_adapter_result_missing"]
    errors: list[str] = []
    if result.get("schema") != "aits_live_preflight_readonly_adapter_contract_v1":
        errors.append("schema_invalid")
    if result.get("adapter_mode") != "readonly_contract":
    ...
    if result.get("adapter_ready") and result.get("blockers"):
        errors.append("adapter_ready_with_blockers")
    return errors


def _build_live_preflight_readonly_adapter_skeleton_result(
    candidate: dict[str, Any],
    *,
    target_symbol: str = "",
    session_approved_symbols: Any = None,
    ...
    mock_total_window_used_krw: int = 0,
    mock_submitted_count: int = 0,
    adapter_input_overrides: dict[str, Any] | None = None,
    adapter_context: dict[str, Any] | None = None,
) -> dict[str, Any]:
    readiness_result = _build_live_preflight_readiness_stub_result(
        candidate,
        target_symbol=target_symbol or str(candidate.get("symbol") or ""),
        session_approved_symbols=session_approved_symbols,
        mock_unlock_approved_symbols=mock_unlock_approved_symbols,
    ...
    adapter_input = _build_live_preflight_readonly_adapter_input_v1(
        readiness_result,
        overrides=adapter_input_overrides,
    )
    adapter_contract = _evaluate_live_preflight_readonly_adapter_contract_v1(
        adapter_input,
        context=adapter_context,
    )
    validation_errors = _validate_live_preflight_readonly_adapter_contract_v1(adapter_contract)
    return {
        "candidate": dict(candidate or {}),
        "live_preflight_readiness_result": readiness_result,
        "adapter_input": adapter_input,
        "adapter_contract": adapter_contract,
        "schema": adapter_contract.get("schema"),
        "target_symbol": adapter_contract.get("symbol"),
        "adapter_ready": bool(adapter_contract.get("adapter_ready")),
        "adapter_mode": adapter_contract.get("adapter_mode"),
        "would_call_live_preflight": bool(adapter_contract.get("would_call_live_preflight")),
        "live_preflight_called": bool(adapter_contract.get("live_preflight_called")),
        "unlock_consumed": bool(adapter_contract.get("unlock_consumed")),
        "order_service_reachable": bool(adapter_contract.get("order_service_reachable")),
        "order_adapter_reachable": bool(adapter_contract.get("order_adapter_reachable")),
        "actual_order_intent_emitted": bool(adapter_contract.get("actual_order_intent_emitted")),
        "decision_router_called": bool(adapter_contract.get("decision_router_called")),
        "risk_guard_called": bool(adapter_contract.get("risk_guard_called")),
        "order_service_called": bool(adapter_contract.get("order_service_called")),
        "order_adapter_called": bool(adapter_contract.get("order_adapter_called")),
        "unlock_service_called": bool(adapter_contract.get("unlock_service_called")),
        "final_action_unchanged": bool(adapter_contract.get("final_action_unchanged")),
        "submitted_count": int(adapter_contract.get("submitted_count") or 0),
        "provider_external_call_count": int(adapter_contract.get("provider_external_call_count") or 0),
        "managed_pool_mutation": bool(adapter_contract.get("managed_pool_mutation")),
        "order_risk_detected": bool(adapter_contract.get("order_risk_detected")),
        "blockers": list(adapter_contract.get("blockers") or []),
        "warnings": list(adapter_contract.get("warnings") or []),
        "validation_errors": validation_errors,
    }


def _run_live_preflight_readonly_adapter_skeleton_fixture_proof(report: dict[str, Any]) -> None:
    valid = _base_source_policy_candidate("user_added", "KRW-PYTH")
    scenarios: list[dict[str, Any]] = [
        {"name": "valid_readiness_chain_adapter_ready", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "expected_ready": True, "expected_blockers": []},
        {"name": "live_preflight_readiness_false_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "overrides": {"live_preflight_readiness": False}, "expected_ready": False, "expected_blockers": ["live_preflight_readiness_not_ready"]},
        {"name": "one_shot_unlock_not_ready_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": [], "expected_ready": False, "expected_blockers": ["one_shot_unlock_not_ready"]},
        {"name": "router_validation_not_ready_blocks", "candidate": {**valid, "intended_amount_krw": 5_000}, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "expected_ready": False, "expected_blockers": ["router_validation_payload_not_ready"]},
        {"name": "source_policy_not_ready_blocks", "candidate": valid, "approved": [], "unlock": ["KRW-PYTH"], "expected_ready": False, "expected_blockers": ["source_policy_not_ready"]},
        {"name": "amount_below_min_blocks", "candidate": {**valid, "intended_amount_krw": 9_000}, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "amount": 9_000, "expected_ready": False, "expected_blockers": ["amount_below_min_order"]},
        {"name": "amount_above_hard_cap_blocks", "candidate": {**valid, "intended_amount_krw": 13_000}, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "amount": 13_000, "expected_ready": False, "expected_blockers": ["amount_above_per_order_hard_cap"]},
        {"name": "total_window_cap_exceeded_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "used": 15_000, "expected_ready": False, "expected_blockers": ["total_window_cap_exceeded"]},
        ...
        {"name": "live_preflight_called_true_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "context": {"live_preflight_called": True}, "expected_ready": False, "expected_blockers": ["live_preflight_called_true"]},
        {"name": "unlock_consumed_true_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "context": {"unlock_consumed": True}, "expected_ready": False, "expected_blockers": ["unlock_consumed_true"]},
        {"name": "order_service_reachable_true_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "context": {"order_service_reachable": True}, "expected_ready": False, "expected_blockers": ["order_service_reachable_true"]},
        {"name": "order_adapter_reachable_true_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "context": {"order_adapter_reachable": True}, "expected_ready": False, "expected_blockers": ["order_adapter_reachable_true"]},
        {"name": "actual_emit_never", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "expected_ready": True, "expected_blockers": []},
        {"name": "router_risk_order_never_called", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "expected_ready": True, "expected_blockers": []},
    ]
    results: list[dict[str, Any]] = []
    for scenario in scenarios:
        result = _build_live_preflight_readonly_adapter_skeleton_result(
            scenario["candidate"],
            target_symbol=str(scenario["candidate"].get("symbol") or ""),
            session_approved_symbols=scenario.get("approved"),
            mock_unlock_approved_symbols=scenario.get("unlock"),
            ...
            mock_total_window_used_krw=int(scenario.get("used") or 0),
            mock_submitted_count=int(scenario.get("submitted") or 0),
            adapter_input_overrides=scenario.get("overrides"),
            adapter_context=scenario.get("context"),
        )
        blockers = set(result.get("blockers") or [])
        passed = bool(result.get("adapter_ready")) == bool(scenario["expected_ready"])
        passed = passed and all(blocker in blockers for blocker in scenario.get("expected_blockers") or [])
        passed = passed and not result.get("validation_errors")
        passed = passed and not any(bool(result.get(key)) for key in ("would_call_live_preflight", "live_preflight_called", "unlock_consumed", "order_service_reachable", "order_adapter_reachable", "actual_order_intent_emitted", "decision_router_called", "risk_guard_called", "order_service_called", "order_adapter_called", "unlock_service_called", "managed_pool_mutation", "order_risk_detected"))
        results.append({"name": scenario["name"], "pass": passed, **result})
    report.update(
        {
            "live_preflight_readonly_adapter_skeleton_supported": True,
            "schema": "aits_live_preflight_readonly_adapter_contract_v1",
            "adapter_helper_owner": "tools/runtime_smoke/aits_qt_smoke_harness.py::_evaluate_live_preflight_readonly_adapter_contract_v1",
            "fixture_results": results,
            "fixture_pass_count": sum(1 for item in results if item.get("pass")),
            "fixture_fail_count": sum(1 for item in results if not item.get("pass")),
            "actual_order_intent_emitted": False,
...
def _run_live_preflight_readonly_adapter_skeleton_live_proof(
    report: dict[str, Any],
    *,
    output_dir: Path,
    target_symbol: str | None = None,
    ...
    embedded: dict[str, Any] = {"mode": "order-intent-live-preflight-readiness-live-proof", "embedded": True}
    ...
    expected_safe = not any(bool(result.get(key)) for key in ("would_call_live_preflight", "live_preflight_called", "unlock_consumed", "order_service_reachable", "order_adapter_reachable", "actual_order_intent_emitted", "decision_router_called", "risk_guard_called", "order_service_called", "order_adapter_called", "unlock_service_called", "managed_pool_mutation", "order_risk_detected"))
    report.update(
        {
            "mode": "live-preflight-readonly-adapter-skeleton-live-proof",
            "live_preflight_readonly_adapter_skeleton_supported": True,
            "schema": "aits_live_preflight_readonly_adapter_contract_v1",
            "adapter_helper_owner": "tools/runtime_smoke/aits_qt_smoke_harness.py::_evaluate_live_preflight_readonly_adapter_contract_v1",
            "target_symbol": str(target_symbol or embedded.get("target_symbol") or result.get("target_symbol") or ""),
            "session_approved_symbols": _normalized_session_approved_symbols(session_approved_symbols),
            "mock_unlock_approved_symbols": _normalized_session_approved_symbols(mock_unlock_approved_symbols),
            "embedded_live_preflight_readiness_report": {
                "status": embedded.get("status"),
                "source_policy_ready": embedded.get("source_policy_ready"),
                "router_validation_payload_ready": embedded.get("router_validation_payload_ready"),
                "one_shot_unlock_ready": embedded.get("one_shot_unlock_ready"),
                "live_order_readiness": embedded.get("live_order_readiness"),
                "live_preflight_readiness": embedded.get("live_preflight_readiness"),
                "policy_blockers": embedded.get("policy_blockers"),
            },
            **result,
        }
...
def _build_riskguard_readonly_adapter_input_v1(
    live_adapter_result: dict[str, Any] | None,
    *,
    overrides: dict[str, Any] | None = None,
) -> dict[str, Any]:
    live_adapter_result = dict(live_adapter_result or {})
    adapter_input = dict(live_adapter_result.get("adapter_input") or {})
    candidate = dict(live_adapter_result.get("candidate") or {})
    target_symbol = _normalize_symbol_text(
        str(adapter_input.get("symbol") or live_adapter_result.get("target_symbol") or candidate.get("symbol") or "")
    )
    intended_amount = int(
        adapter_input.get("intended_amount_krw")
        or candidate.get("intended_amount_krw")
        or live_adapter_result.get("intended_amount_krw")
        or 0
    )
    total_window_used = int(live_adapter_result.get("mock_total_window_used_krw") or 0)
    total_after = int(adapter_input.get("total_window_after_candidate_krw") or 0)
    if total_after >= intended_amount:
        total_window_used = total_after - intended_amount
    payload = {
        "schema": "aits_riskguard_readonly_adapter_input_v1",
        "target_symbol": target_symbol,
        "source": str(adapter_input.get("source") or candidate.get("managed_source") or candidate.get("source") or "unknown"),
        "managed_source": str(candidate.get("managed_source") or adapter_input.get("source") or "unknown"),
        "intended_side": str(adapter_input.get("side") or candidate.get("side") or "buy"),
        "intended_amount_krw": intended_amount,
        "min_order_krw": int(adapter_input.get("min_order_krw") or candidate.get("min_order_krw") or 10_000),
        "per_order_hard_cap_krw": int(adapter_input.get("per_order_hard_cap_krw") or candidate.get("per_order_hard_cap_krw") or 12_000),
        "total_guarded_window_cap_krw": int(adapter_input.get("total_window_cap_krw") or candidate.get("total_window_cap_krw") or 20_000),
        "mock_total_window_used_krw": total_window_used,
        "submitted_count": int(adapter_input.get("submitted_count") or live_adapter_result.get("submitted_count") or 0),
        "session_approved_symbols": list(live_adapter_result.get("session_approved_symbols") or []),
        "ai_opinion": str(candidate.get("ai_opinion") or candidate.get("status_label") or ""),
        "ai_opinion_freshness_status": str(candidate.get("ai_freshness") or candidate.get("freshness") or ""),
        "router_validation_payload_ready": bool(adapter_input.get("router_validation_payload_ready")),
        "source_policy_ready": bool(adapter_input.get("source_policy_ready")),
        "one_shot_unlock_ready": bool(adapter_input.get("one_shot_unlock_ready")),
        "live_preflight_readiness": bool(adapter_input.get("live_preflight_readiness")),
        "live_preflight_adapter_ready": bool(live_adapter_result.get("adapter_ready")),
        "duplicate_guard_required": bool(adapter_input.get("duplicate_guard_required")),
        "repeat_guard_required": bool(adapter_input.get("repeat_guard_required")),
        "relock_required": bool(adapter_input.get("relock_required")),
        "actual_order": False,
        "actual_order_intent_emitted": False,
        "final_action_unchanged": True,
        "validation_only": True,
...
def _evaluate_riskguard_readonly_adapter_contract_v1(
    adapter_input: dict[str, Any] | None,
    *,
    context: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    if str(adapter_input.get("schema") or "") != "aits_riskguard_readonly_adapter_input_v1":
        blockers.append("adapter_input_schema_invalid")
    ...
    required_truthy = {
        ...
        "one_shot_unlock_ready": "one_shot_unlock_not_ready",
        "live_preflight_readiness": "live_preflight_readiness_not_ready",
        "live_preflight_adapter_ready": "live_preflight_adapter_not_ready",
        "duplicate_guard_required": "duplicate_guard_missing",
        "repeat_guard_required": "repeat_guard_missing",
        "relock_required": "relock_missing",
        "validation_only": "validation_only_missing",
    ...
    risk_input_ready = bool(target_symbol and intended_amount and adapter_input.get("schema") == "aits_riskguard_readonly_adapter_input_v1")
    policy_blockers = [item for item in blockers if item not in {"target_symbol_missing", "adapter_input_schema_invalid", "validation_only_missing"}]
    risk_policy_ready = not policy_blockers
    blockers = list(dict.fromkeys(blockers))
    warnings = list(dict.fromkeys(warnings))
    adapter_ready = bool(risk_input_ready and risk_policy_ready and not blockers)

    return {
        "schema": "aits_riskguard_readonly_adapter_contract_v1",
        "input_schema": str(adapter_input.get("schema") or ""),
        "target_symbol": target_symbol,
        ...
        "intended_amount_krw": intended_amount,
        "adapter_ready": adapter_ready,
        "riskguard_adapter_ready": adapter_ready,
        "adapter_mode": "readonly_contract",
        ...
        "managed_pool_mutation": False,
        "order_risk_detected": False,
        "required_next_goal": "AITS-LIVE-PREFLIGHT-READONLY-ADAPTER-SKELETON-PROOF-01",
    }


def _validate_riskguard_readonly_adapter_contract_v1(result: dict[str, Any] | None) -> list[str]:
    if not result:
        return ["riskguard_readonly_adapter_result_missing"]
    errors: list[str] = []
    if result.get("schema") != "aits_riskguard_readonly_adapter_contract_v1":
        errors.append("schema_invalid")
    if result.get("adapter_mode") != "readonly_contract":
        errors.append("adapter_mode_invalid")
    ...
    if int(result.get("provider_external_call_count") or 0) != 0:
        errors.append("provider_call_count_must_be_zero")
    if result.get("adapter_ready") and result.get("risk_blockers"):
        errors.append("adapter_ready_with_blockers")
    return errors
...
def _build_riskguard_readonly_adapter_skeleton_result(
    ...
    *,
    ...
    live_adapter_result = _build_live_preflight_readonly_adapter_skeleton_result(
        candidate,
        target_symbol=target_symbol or str(candidate.get("symbol") or ""),
        session_approved_symbols=session_approved_symbols,
        mock_unlock_approved_symbols=mock_unlock_approved_symbols,
    ...
    live_adapter_result["session_approved_symbols"] = _normalized_session_approved_symbols(session_approved_symbols)
    live_adapter_result["mock_total_window_used_krw"] = int(mock_total_window_used_krw or 0)
    adapter_input = _build_riskguard_readonly_adapter_input_v1(live_adapter_result, overrides=adapter_input_overrides)
    adapter_contract = _evaluate_riskguard_readonly_adapter_contract_v1(adapter_input, context=adapter_context)
    validation_errors = _validate_riskguard_readonly_adapter_contract_v1(adapter_contract)
    return {
        "candidate": dict(candidate or {}),
        "live_preflight_readonly_adapter_result": live_adapter_result,
        "adapter_input": adapter_input,
        "adapter_contract": adapter_contract,
        "schema": adapter_contract.get("schema"),
        "target_symbol": adapter_contract.get("target_symbol"),
        "adapter_ready": bool(adapter_contract.get("adapter_ready")),
        "riskguard_adapter_ready": bool(adapter_contract.get("riskguard_adapter_ready")),
        "adapter_mode": adapter_contract.get("adapter_mode"),
        "would_call_riskguard": bool(adapter_contract.get("would_call_riskguard")),
        "risk_guard_called": bool(adapter_contract.get("risk_guard_called")),
        "risk_decision": adapter_contract.get("risk_decision"),
        "risk_guard_result_present": bool(adapter_contract.get("risk_guard_result_present")),
        "risk_guard_reachable": bool(adapter_contract.get("risk_guard_reachable")),
        "risk_input_ready": bool(adapter_contract.get("risk_input_ready")),
        "risk_policy_ready": bool(adapter_contract.get("risk_policy_ready")),
        "risk_blockers": list(adapter_contract.get("risk_blockers") or []),
        "risk_warnings": list(adapter_contract.get("risk_warnings") or []),
        "actual_order": bool(adapter_contract.get("actual_order")),
        "actual_order_intent_emitted": bool(adapter_contract.get("actual_order_intent_emitted")),
        "decision_router_called": bool(adapter_contract.get("decision_router_called")),
        "would_call_live_preflight": bool(adapter_contract.get("would_call_live_preflight")),
        "live_preflight_called": bool(adapter_contract.get("live_preflight_called")),
...
def _run_riskguard_readonly_adapter_skeleton_fixture_proof(report: dict[str, Any]) -> None:
    valid = _base_source_policy_candidate("user_added", "KRW-PYTH")
    scenarios: list[dict[str, Any]] = [
        {"name": "valid_contract_adapter_ready", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "expected_ready": True, "expected_blockers": []},
        ...
        {"name": "total_window_cap_exceeded_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "used": 15_000, "expected_ready": False, "expected_blockers": ["total_guarded_window_cap_exceeded"]},
        ...
        {"name": "actual_emit_detected_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "overrides": {"actual_order_intent_emitted": True}, "expected_ready": False, "expected_blockers": ["actual_emit_detected"]},
        ...
        {"name": "risk_guard_called_true_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "context": {"risk_guard_called": True}, "expected_ready": False, "expected_blockers": ["risk_guard_called_true"]},
        {"name": "live_preflight_called_true_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "context": {"live_preflight_called": True}, "expected_ready": False, "expected_blockers": ["live_preflight_called_true"]},
        {"name": "unlock_consumed_true_blocks", "candidate": valid, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "context": {"unlock_consumed": True}, "expected_ready": False, "expected_blockers": ["unlock_consumed_true"]},
    ]
    forced_false_keys = (
    ...
    results: list[dict[str, Any]] = []
    for scenario in scenarios:
        result = _build_riskguard_readonly_adapter_skeleton_result(
            scenario["candidate"],
            target_symbol=str(scenario["candidate"].get("symbol") or ""),
            session_approved_symbols=scenario.get("approved"),
            mock_unlock_approved_symbols=scenario.get("unlock"),
        ...
        blockers = set(result.get("risk_blockers") or [])
        passed = bool(result.get("adapter_ready")) == bool(scenario["expected_ready"])
        passed = passed and all(blocker in blockers for blocker in scenario.get("expected_blockers") or [])
    ...
    report.update(
        {
            "riskguard_readonly_adapter_skeleton_supported": True,
            "schema": "aits_riskguard_readonly_adapter_contract_v1",
            "adapter_helper_owner": "tools/runtime_smoke/aits_qt_smoke_harness.py::_evaluate_riskguard_readonly_adapter_contract_v1",
            "fixture_results": results,
            "fixture_pass_count": sum(1 for item in results if item.get("pass")),
            "fixture_fail_count": sum(1 for item in results if not item.get("pass")),
            "actual_order": False,
...
def _run_riskguard_readonly_adapter_skeleton_live_proof(
    report: dict[str, Any],
    *,
    output_dir: Path,
    target_symbol: str | None = None,
    ...
    embedded: dict[str, Any] = {"mode": "live-preflight-readonly-adapter-skeleton-live-proof", "embedded": True}
    _run_live_preflight_readonly_adapter_skeleton_live_proof(
        embedded,
        output_dir=output_dir,
        target_symbol=target_symbol,
        min_score=min_score,
    ...
    report.update(
        {
            "mode": "riskguard-readonly-adapter-skeleton-live-proof",
            "riskguard_readonly_adapter_skeleton_supported": True,
            "schema": "aits_riskguard_readonly_adapter_contract_v1",
            "adapter_helper_owner": "tools/runtime_smoke/aits_qt_smoke_harness.py::_evaluate_riskguard_readonly_adapter_contract_v1",
            "target_symbol": str(target_symbol or embedded.get("target_symbol") or result.get("target_symbol") or ""),
            "session_approved_symbols": _normalized_session_approved_symbols(session_approved_symbols),
            "mock_unlock_approved_symbols": _normalized_session_approved_symbols(mock_unlock_approved_symbols),
            "embedded_live_preflight_readonly_adapter_report": {
                "status": embedded.get("status"),
...
def _evaluate_live_preflight_readonly_actual_adapter_contract_v1(
    ...
    *,
    ...
    required_truthy = {
        ...
        "relock_required": "relock_missing",
        "static_callable_inspection_only": "static_callable_inspection_only_missing",
        "live_preflight_callable_identified": "live_preflight_callable_not_identified",
        "live_preflight_input_contract_ready": "live_preflight_input_contract_not_ready",
        "live_preflight_output_contract_ready": "live_preflight_output_contract_not_ready",
        "live_preflight_call_boundary_defined": "live_preflight_call_boundary_not_defined",
        "live_preflight_side_effect_boundary_defined": "live_preflight_side_effect_boundary_not_defined",
        "live_preflight_mutation_forbidden": "live_preflight_mutation_boundary_not_defined",
        "validation_only": "validation_only_missing",
    }
    for key, blocker in required_truthy.items():
        if not bool(adapter_input.get(key)):
    ...
    forced_false_flags = (
        "would_call_live_preflight",
        "live_preflight_called",
        "live_preflight_result_present",
        "live_preflight_reachable",
        "would_call_riskguard",
        "risk_guard_called",
        "decision_router_called",
        "order_service_called",
    ...
    return {
        ...
        "live_preflight_input_contract_ready": bool(adapter_input.get("live_preflight_input_contract_ready")),
        "live_preflight_output_contract_ready": bool(adapter_input.get("live_preflight_output_contract_ready")),
        "live_preflight_call_boundary_defined": bool(adapter_input.get("live_preflight_call_boundary_defined")),
        "live_preflight_side_effect_boundary_defined": bool(adapter_input.get("live_preflight_side_effect_boundary_defined")),
        "live_preflight_mutation_forbidden": bool(adapter_input.get("live_preflight_mutation_forbidden")),
        "would_call_live_preflight": False,
        "live_preflight_called": False,
...
def _validate_live_preflight_readonly_actual_adapter_contract_v1(result: dict[str, Any] | None) -> list[str]:
    if not result:
        return ["live_preflight_readonly_actual_adapter_result_missing"]
    errors: list[str] = []
    if result.get("schema") != "aits_live_preflight_readonly_actual_adapter_contract_v1":
        errors.append("schema_invalid")
    if result.get("adapter_mode") != "actual_readonly_contract":
        errors.append("adapter_mode_invalid")
    if result.get("live_preflight_decision") != "not_evaluated":
        errors.append("live_preflight_decision_must_be_not_evaluated")
    for key in (
        "would_call_live_preflight",
        "live_preflight_called",
        "live_preflight_result_present",
    ...
    if int(result.get("provider_external_call_count") or 0) != 0:
        errors.append("provider_call_count_must_be_zero")
    if result.get("live_preflight_actual_readonly_adapter_ready") and result.get("blockers"):
        errors.append("live_preflight_actual_readonly_adapter_ready_with_blockers")
    return errors
...
def _build_live_preflight_readonly_actual_adapter_result(
    ...
    *,
    ...
    return {
        ...
        "schema": actual_contract.get("schema"),
        "target_symbol": actual_contract.get("target_symbol"),
        "live_preflight_actual_readonly_adapter_ready": bool(actual_contract.get("live_preflight_actual_readonly_adapter_ready")),
        "adapter_ready": bool(actual_contract.get("adapter_ready")),
        "adapter_mode": actual_contract.get("adapter_mode"),
        "live_preflight_callable_identified": bool(actual_contract.get("live_preflight_callable_identified")),
        "live_preflight_callable_name": actual_contract.get("live_preflight_callable_name"),
        "live_preflight_input_contract_ready": bool(actual_contract.get("live_preflight_input_contract_ready")),
        "live_preflight_output_contract_ready": bool(actual_contract.get("live_preflight_output_contract_ready")),
        "live_preflight_call_boundary_defined": bool(actual_contract.get("live_preflight_call_boundary_defined")),
        "live_preflight_side_effect_boundary_defined": bool(actual_contract.get("live_preflight_side_effect_boundary_defined")),
        "live_preflight_mutation_forbidden": bool(actual_contract.get("live_preflight_mutation_forbidden")),
        "would_call_live_preflight": bool(actual_contract.get("would_call_live_preflight")),
        "live_preflight_called": bool(actual_contract.get("live_preflight_called")),
        "live_preflight_decision": actual_contract.get("live_preflight_decision"),
        "live_preflight_result_present": bool(actual_contract.get("live_preflight_result_present")),
        "live_preflight_reachable": bool(actual_contract.get("live_preflight_reachable")),
        "would_call_riskguard": bool(actual_contract.get("would_call_riskguard")),
        "risk_guard_called": bool(actual_contract.get("risk_guard_called")),
        "decision_router_called": bool(actual_contract.get("decision_router_called")),
        "order_service_reachable": bool(actual_contract.get("order_service_reachable")),
...
def _run_live_preflight_readonly_actual_adapter_fixture_proof(report: dict[str, Any]) -> None:
    ...
    report.update(
        {
            "live_preflight_readonly_actual_adapter_supported": True,
            "schema": "aits_live_preflight_readonly_actual_adapter_contract_v1",
            "adapter_helper_owner": "tools/runtime_smoke/aits_qt_smoke_harness.py::_evaluate_live_preflight_readonly_actual_adapter_contract_v1",
            "callable_contract_source": "static_design_contract",
            "fixture_results": results,
            "fixture_pass_count": sum(1 for item in results if item.get("pass")),
            "fixture_fail_count": sum(1 for item in results if not item.get("pass")),
...
def _run_live_preflight_readonly_actual_adapter_live_proof(
    ...
    *,
    ...
    report.update(
        {
            "mode": "live-preflight-readonly-actual-adapter-live-proof",
            "live_preflight_readonly_actual_adapter_supported": True,
            "schema": "aits_live_preflight_readonly_actual_adapter_contract_v1",
            "adapter_helper_owner": "tools/runtime_smoke/aits_qt_smoke_harness.py::_evaluate_live_preflight_readonly_actual_adapter_contract_v1",
            "callable_contract_source": "static_design_contract",
            ...
            "session_approved_symbols": _normalized_session_approved_symbols(session_approved_symbols),
            "mock_unlock_approved_symbols": _normalized_session_approved_symbols(mock_unlock_approved_symbols),
            "embedded_riskguard_readonly_actual_adapter_report": {
                "status": embedded.get("status"),
                "actual_readonly_adapter_ready": embedded.get("actual_readonly_adapter_ready"),
                "would_call_riskguard": embedded.get("would_call_riskguard"),
                "risk_guard_called": embedded.get("risk_guard_called"),
                "risk_decision": embedded.get("risk_decision"),
                "risk_result_present": embedded.get("risk_result_present"),
                "blockers": embedded.get("blockers"),
                "validation_errors": embedded.get("validation_errors"),
            },
            **result,
...
def _evaluate_live_minimal_order_armed_contract_v1(
    ...
    *,
    ...
    if total_after > total_cap:
        blockers.append("configured_amount_exceeds_guarded_window_cap")
    if submitted_count != 0:
        blockers.append("submitted_count_not_zero")
    if managed_source == "user_added" and target_symbol not in session_approved:
        blockers.append("user_added_not_session_approved")
...
def _build_live_minimal_order_armed_result(
    ...
    *,
    ...
    return {
        ...
        "amount_locked_to_current_setting": bool(armed_contract.get("amount_locked_to_current_setting")),
        "amount_locked_to_10000_deprecated": bool(armed_contract.get("amount_locked_to_10000_deprecated")),
...
def _run_live_minimal_order_armed_fixture_proof(
    ...
    *,
    ...
    scenarios: list[dict[str, Any]] = [
        {"name": "requested_setting_amount_case", "candidate": {**valid, "intended_amount_krw": int(intended_amount_krw or 0)}, "approved": ["KRW-PYTH"], "unlock": ["KRW-PYTH"], "amount": int(intended_amount_krw or 0), "configured": int(configured_order_amount_krw or 0), "phrase": str(operator_confirm_phrase or ""), "submitted": int(mock_submitted_count or 0), "expected_armed": requested_expected_armed, "expected_blockers": requested_expected_blockers},
...
def _live_on_runtime_e2e_status_from_blocker(blocker: str) -> str:
    mapping = {
        ...
        "user_app_session_missing": "no_runtime",
...
def _live_on_runtime_e2e_next_fix_target(blocker: str) -> str:
    mapping = {
        ...
        "user_app_session_missing": "ask user to launch app manually and wait for feed logs",
...
def run_harness(
    ...
    *,
    ...
    mock_unlock_approved_symbols: str = "",
    mock_unlock_expired: bool = False,
    mock_unlock_consumed: bool = False,
    mock_unlock_reusable: bool = False,
    intended_amount_krw: int = 10_000,
    ...
    if mode in {
        ...
        if mode == "riskguard-proof":
        ...
        elif mode == "real-user-app-public-feed-profile-summary":
            _install_provider_post_guard(report)
            _run_real_user_app_public_feed_profile_summary(report, output_dir=output_dir)
        ...
        elif mode == "order-intent-one-shot-unlock-readiness-fixture-proof":
            _run_order_intent_one_shot_unlock_readiness_fixture_proof(report)
        elif mode == "order-intent-one-shot-unlock-readiness-live-proof":
            _install_provider_post_guard(report)
            _run_order_intent_one_shot_unlock_readiness_live_proof(
                report,
                output_dir=output_dir,
        ...
        elif mode == "live-preflight-readonly-adapter-skeleton-fixture-proof":
            _run_live_preflight_readonly_adapter_skeleton_fixture_proof(report)
        elif mode == "live-preflight-readonly-adapter-skeleton-live-proof":
            _install_provider_post_guard(report)
            _run_live_preflight_readonly_adapter_skeleton_live_proof(
                report,
                output_dir=output_dir,
        ...
        elif mode == "live-preflight-readonly-actual-adapter-fixture-proof":
            _run_live_preflight_readonly_actual_adapter_fixture_proof(report)
            ...
            _run_live_minimal_order_armed_fixture_proof(
                report,
            ...
            _install_provider_post_guard(report)
            _run_asset_position_policy_inheritance_summary(report)
        elif mode == "asset-position-policy-ai-dynamic-summary":
            _install_provider_post_guard(report)
            _run_asset_position_policy_inheritance_summary(report)
        elif mode == "live-on-preflight-ai-dynamic-cap-summary":
            _install_provider_post_guard(report)
            _run_live_on_preflight_effective_cap_summary(report)
            report["schema"] = "aits_live_on_preflight_ai_dynamic_cap_summary_v1"
...
def main() -> int:
    ...
    parser.add_argument("--mock-unlock-approved-symbols", default="")
    ...
    parser.add_argument("--configured-order-amount-krw", type=int)
    ...
    report = run_harness(
        ...
        mock_unlock_approved_symbols=args.mock_unlock_approved_symbols,
        ...
        configured_order_amount_krw=args.configured_order_amount_krw,
